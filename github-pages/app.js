...
function drawText(context, commit, text=textInput.value, targetSlot=slot, fixedPalette=null) {
  ...
  const image=tileContext.getImageData(0,0,size,size);let maxOpaqueX=-1,maxOpaqueY=0;for(let index=0;index<image.data.length;index+=4){image.data[index+3]=image.data[index+3]>=96?255:0;if(image.data[index+3]===255){const pixel=index/4,x=pixel%size,y=Math.floor(pixel/size);if(x>maxOpaqueX){maxOpaqueX=x;maxOpaqueY=y;}}}if(sequenceCell&&maxOpaqueX>=0&&rightGap>0&&maxOpaqueX+rightGap<size){const marker=(maxOpaqueY*size+maxOpaqueX+rightGap)*4;image.data[marker]=0;image.data[marker+1]=0;image.data[marker+2]=0;image.data[marker+3]=1;}tileContext.putImageData(image,0,0);
